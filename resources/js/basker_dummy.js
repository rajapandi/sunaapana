...
function baby_cont(url, imgid, dv) {
    dv1 = dv;
    document.getElementById("d").style.display = "block";
    document.getElementById(dv).innerHTML = '<img src="images/ajax-loader.gif" align="absmiddle"/>loading ...';
    imgid1 = imgid;
    if (url.length == 0) {
        return;
    }
...
function rating() {
    ...
    baby_cont('AddCat_user.php', getCheckedValue("poll"), 'apDiv2');
}
//--------- email
function mail() {
    var email = document.getElementById("emailid").value;
    baby_cont('ajaxmailsend.php', email, 'dd');
...
function ss() {
    if (document.getElementById("inputString").value != "") {
        ...
        if (twins != undefined) {
            str_qs = str_qs + "&t=" + twins
        } else {
            str_qs = str_qs + "&t=0"
        }
        if ($('#num').val() != "") {
            str_qs = str_qs + "&numeroval=" + $('#num').val()
        } else {
            str_qs = str_qs + "&numeroval=0"
        }
        window.location.href = "search.php?key=" + document.getElementById("inputString").value + str_qs
    }
}
...
function CursorKeyDown(e, inputString) {
    if (!e)
        e = window.event;
    Numnode = "";
    if (document.getElementById('List')) {
        var Numnode = document.getElementById('List').childNodes.length - 1
    }
    switch (e.keyCode) {
        case 38:
            if (i > 1) {
            ...
            for (jj = 1; jj <= Numnode; jj++) {
                document.getElementById("l" + jj).style.backgroundColor = "#fff"
            }
            document.getElementById("l" + i).style.backgroundColor = "#e9ffd3";
            var str = document.getElementById("l" + i).innerHTML;
            str = str.replace("<b>", "");
            str = str.replace("</b>", "");
            str = str.replace("<B>", "");
            str = str.replace("</B>", "");
            str = str.split(" (");
            $('#inputString').val(str[0]);
            break;
        case 40:
            if (i < Numnode) {
                i = i + 1
            }
            for (kk = 1; kk < Numnode; kk++) {
                document.getElementById("l" + kk).style.backgroundColor = "#fff"
            }
            document.getElementById("l" + i).style.backgroundColor = "#e9ffd3";
            var str = document.getElementById("l" + i).innerHTML;
            str = str.replace("<b>", "");
...
$(document).ready(function () {
    ...
    if ($.cookie("BabyBasket") == "open") {
        $("#contentWrapRight").show();
        $("#c_wrp").hide()
    } else if ($.cookie("BabyBasket") == "close") {
        $("#contentWrapRight").hide();
        $("#c_wrp").show()
    }
    ...
    $("#Bcls").click(function () {
        $("#contentWrapRight").hide();
        $("#c_wrp").show();
        $.cookie("BabyBasket", "close")
    });
    $("#crtbtn").click(function () {
        $("#c_wrp").hide();
        $("#contentWrapRight").show();
        $.cookie("BabyBasket", "open")
    });
    ...
    $(".BnWR a img").click(function () {
        $("#c_wrp").hide();
        $("#contentWrapRight").show();
        $.cookie("BabyBasket", "open");
        var b = (this.id).split("_");
        var c = b[1];
        var d = $("#babynameWpId_" + c).offset().left;
        var e = $("#babynameWpId_" + c).offset().top;
        if ($("#productID_" + c).length > 1) {
            var f = $("#productID_" + c).offset().left;
            var g = $("#productID_" + c).offset().top
        } else {
        ...
        var h = f - d;
        var i = g - e;
        var j = $("#babynameWpId_" + c).width() / 3;
        var k = $("#babynameWpId_" + c).height() / 3;
        $("#babynameWpId_" + c + " img").clone().prependTo("#babynameWpId_" + c).css({
            'position': 'absolute',
            'text-align': 'center',
            'height': '256px'
        }).animate({
            ...
            $.ajax({
                ...
                success: function (a) {
                    if ($("#productID_" + c).length > 0) {
                        ...
                        $("#basketItemsWrap li:first").before(a);
                        $("#basketItemsWrap li:first").hide();
    ...
    $("#basketItemsWrap li a img").live("click", function (b) {
        ...
        $.ajax({
            ...
            success: function (a) {
                $("#productID_" + d).hide("slow", function () {
                    $(this).remove();
                    $("#slctd").html($("#basketItemsWrap li").length - 1)
                });
                $("#notificationsLoader").empty()
    ...
    $("#cb a").click(function () {
        ...
        $("#frameDiv").html("<div id='iframeClose'>x</div><iframe src='" + a + "' height='300px' width='300px' scrolling='no'  style='border:none;'></iframe>")
    });
    $("#iframeClose").live("click", function (a) {
        $("#divbg").hide();
        $("#frameDiv").hide();
        $('html, body').css('overflow', 'auto')
    });
