...
class Names_model extends CI_Model {
    ...
    function get_popular_names($category = NULL, $gender = NULL, $limit = 5) {
        $this->db->select("n.*");
    ...
    function set_like($name_id, $mode) {
        $this->db->where('baby_name_id', $name_id);
        $like = ($mode == 'l') ? "likes+1" : "likes-1";
        $this->db->set('likes', $like, FALSE);
        return $this->db->update("baby_name");
    }

    function getSingleMeaning($id) {
        $this->db->select('baby_name_means AS meanings');
    ...
    function getTags($id) {
        $this->db->select('GROUP_CONCAT(c.id) AS tags, GROUP_CONCAT(c.name) AS tags_list');
    ...
    function getNamesByNumerology($category = NULL, $gender = NULL, $starts_with = NULL, $initial_index = NULL, $date_nuerology = NULL, $per_page = null, $page = null) {
        ...
        $this->db->select("n.*, (n.numerology + $initial_index) as new_numberology");
        // $this->db->select("*");
        $this->db->from('baby_name n');
        ...
        /* if ($numerology > 0)
          $this->db->where('n.numerology', $numerology); */
        ...
        if (!empty($starts_with)) {
            ...
            $starts_with = array_map("trim", $starts_with);


            $i = 0;
            foreach ($starts_with as $item) { // loop column 
                $item = ltrim($item);
                $item = rtrim($item);
                if ($i === 0) { // first loop
                    $this->db->group_start(); // open bracket. query Where with OR clause better with bracket. because maybe can combine with other WHERE with AND.
                    $this->db->like('n.baby_name', $item, 'after');
                } else {
                    $this->db->or_like('n.baby_name', $item, 'after');
                }

                if (count($starts_with) - 1 == $i) //last loop
        ...
        $this->db->having('new_numberology', $date_nuerology);
    ...
    function getDoubleNamesByNumerology($category = NULL, $gender = NULL, $starts_with = NULL, $initial_index = NULL, $date_nuerology = NULL, $per_page = null, $page = null) {
        ...
        $this->db->select("n.*, m.baby_name as second_name, (n.numerology + $initial_index) as new_numberology");
        // $this->db->select("*");
        $this->db->from('baby_name n, baby_name m');
        //$this->db->join('baby_name_meaning m', 'n.baby_name_id = m.baby_name_id', 'LEFT');
        $this->db->join('baby_name_tags t', 'n.baby_name_id = t.baby_name_id', 'LEFT');
        $this->db->join('category c', 't.baby_tag_id = c.id', 'LEFT');
    ...
    function getNames($category = NULL, $gender = NULL, $starts_with = NULL, $per_page = null, $page = null) {
        ...
        $this->db->group_by('n.baby_name_id');
